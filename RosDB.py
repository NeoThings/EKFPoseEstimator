...
# ODOM = '/odom'
# IMU = '/imu/data'
ODOM = '/dev/odometry/raw/dist'
IMU = '/dev/imu/raw/dist'
...
class RosDB:
    ...
    def get_odom_values(self):
        ...
        odometry = {
            ...
            ORIENTATION: np.zeros([message_count, 3]),  # quaternion convert to euler angles
            LINEAR_T: np.zeros([message_count, 3]),
            ANGULAR_T: np.zeros([message_count, 3]), 
        ...
        index = 0
        start_time = None
        portion = max(1, message_count // 10)
    ...
    def get_imu_values(self):
        ...
        # -- Initialize the matrix --
        imu = {ORIENTATION: np.zeros([message_count, 3]),  # quaternion convert to euler angles
